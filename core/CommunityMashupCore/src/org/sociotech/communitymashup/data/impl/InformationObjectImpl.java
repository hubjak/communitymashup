...
public abstract class InformationObjectImpl extends ItemImpl implements InformationObject {
	...
	public EList<Citation> getCitations() {
		...
		try {
			oclCondition = new BooleanOCLCondition<EClassifier, EClass, EObject>( 	getOclEnvironment(),
																					oclStatement,
																					DataPackageImpl.eINSTANCE.getCitation());		
		}
		catch (ParserException e) {
			log("Malformed ocl statement: " + oclStatement + " (" + e.getMessage() + ")", LogService.LOG_ERROR);
			return null;
		...
		IQueryResult result = DataPackageImpl.filterItemsMatchingCondition(getMetaInformations(), oclCondition.AND(Citation.isTypeCondition));

		if(result == null) {
			return new BasicEList<Citation>();
		}
		
		// results are only Citations
		@SuppressWarnings("unchecked")
		EList<Citation> objects = new BasicEList<Citation>((Collection<? extends Citation>) result.getEObjects());
	 ...
	 * @generated
	 */
	public Citation addCitation(String citationData) {
		if(citationData == null || citationData.equals(""))
		{
			return null;
		}
		...
		// set the citation data
		citation.setCitationData(citationData);
		
		// attach it to this information object
		return (Citation) this.extend(citation);
	}
	...
	public Object eInvoke(int operationID, EList<?> arguments) throws InvocationTargetException {
		switch (operationID) {
			...
			case DataPackage.INFORMATION_OBJECT___GET_CITATIONS:
				return getCitations();
			...
			case DataPackage.INFORMATION_OBJECT___ADD_CITATION__STRING:
				return addCitation((String)arguments.get(0));
	...
	protected Object doOperation(RestCommand command) throws ArgNotFoundException, WrongArgException, WrongArgCountException, UnknownOperationException {
		...
		if ( command.getCommand().equalsIgnoreCase("getCitations")) {
			if (command.getArgCount() != 0) throw new WrongArgCountException("InformationObject.doOperation", 0, command.getArgCount()); 
		...
		if ( command.getCommand().equalsIgnoreCase("addCitation")) {
			if (command.getArgCount() != 1) throw new WrongArgCountException("InformationObject.doOperation", 1, command.getArgCount()); 
			java.lang.String citationData = null;
			try {
				citationData = (java.lang.String)command.getArg("citationData");
			} catch (ClassCastException e) {
				throw new WrongArgException("InformationObject.doOperation", "java.lang.String", command.getArg("citationData").getClass().getName());
			}
			return this.addCitation(citationData);
		}
