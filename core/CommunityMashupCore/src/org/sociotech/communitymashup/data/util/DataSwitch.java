...
public class DataSwitch<T> extends Switch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			...
			case DataPackage.CITATION: {
				Citation citation = (Citation)theEObject;
				T result = caseCitation(citation);
				if (result == null) result = caseMetaInformation(citation);
				if (result == null) result = caseExtension(citation);
				if (result == null) result = caseItem(citation);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
