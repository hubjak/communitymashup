...
public class ResearchGateTransformation {
	...
	public void transformAndAddObjects(List<Node> list, Date lastUpdated) {
		...
		String minYear = source.getConfiguration().getPropertyValueElseDefault(ResearchGateProperties.MIN_YEAR_PROPERTY, ResearchGateProperties.MIN_YEAR_DEFAULT);
		int minYearInt = 0;
		try {
		...
		for (Node node: objects){
			...
			temp.metaTag(ResearchGateTags.RESEARCHGATE);		
			...
			if (tmps!=null && tmps.length()>0) {
				isPub = false;
				temp.metaTag(ResearchGateTags.PROJECT);
			} else {
				tmps = node.getAttribute("title");
				if (tmps==null || tmps.length()<1) {
					continue;
				}
				temp.metaTag(ResearchGateTags.PUBLICATION);
			}
